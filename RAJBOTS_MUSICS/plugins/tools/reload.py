...
from SIMPLE_MUSIC.utils.database import get_assistant, get_authuser_names, get_cmode
from SIMPLE_MUSIC.utils.decorators import ActualAdminCB, AdminActual, language
from SIMPLE_MUSIC.utils.formatters import alpha_to_int, get_readable_time
...
async def restartbot(client, message: Message, _):
    ...
    if chat_id:
        ...
        try:
            db[chat_id] = []
            await SIMPLE.stop_stream_force(chat_id)
        except:
            pass
             ...
             [
                 [
                      InlineKeyboardButton(
                         "• нαϲкє𝚍 ву  •", url=f"https://t.me/Rajbots", **_get_style(r1))
                 ]
            ]
         ),
     )
